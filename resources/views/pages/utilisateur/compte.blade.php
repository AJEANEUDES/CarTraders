...
<section class="shop section container">
    <h2 class="breadcrumb__title" style="font-size: 16px;">Bienvenue {{ $client->nom_user }} {{ $client->prenom_user }}
        nous sommes ravi de vous revoir</h2>
    <h3 class="breadcrumb__subtitle">Accueil > <span>Informations personnelle</span></h3>
    ...
    <div class="shop__container grid">
        <div class="sidebar">
            ...
            <div class="filter__content" style="text-align: center;">
                ...
                <h3 class="filter__subtitle">{{ $client->nom_user }} {{ $client->prenom_user }}</h3>
            </div>
            <div class="filter__content">
                <div class="filter" style="cursor: pointer;">
                    <i style="font-size: 22px;" class="bx bx-chevron-right"></i>
                    <p style="font-size: 20px;">
                        <a href="{{ route('utilisateur.profile') }}" style="color: #A40000;">Mon compte</a>
                    </p><span></span>
                </div>
                <div class="filter" style="cursor: pointer;">
                    <i style="font-size: 22px;" class="bx bx-chevron-right"></i>
                    <p style="font-size: 20px;">
                        <a href="{{ route('reservations.liste') }}" style="color: #463429;">Mes reservations</a>
                    </p><span></span>
                </div>
                <div class="filter" style="cursor: pointer;">
                    <i style="font-size: 22px;" class="bx bx-chevron-right"></i>
        ...
        <form action="{{ route('utilisateur.update') }}" method="post" class="contact__form grid" enctype="multipart/form-data" id="utilisateur-update">
            @csrf
            <input type="hidden" name="id_client" id="id_client" value="{{ encodeId($client->id) }}">
            <div class="contact__inputs grid">
                ...
                <div class="contact__content">
                    <label for="prenom_user" class="contact__label">Prenom</label>
                    <input type="text" value="" class="contact__input" name="prenom_user" id="prenom_user"/>
                </div>
            </div>
            ...
            <div class="contact__inputs grid">
                <div class="contact__content">
                    <label for="pays_user" class="contact__label">Email</label>
...
@push('scripts-inscription-check-countries')
    <script>
        $(document).ready(function(){
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route('clients.infos') }}',
                dataType: 'JSON',
                data: {id_client: id_client},
                success: (response)=>{
                    $("#utilisateur-update").find('input[name="nom_user"]').val(response.nom_user);
                    $("#utilisateur-update").find('input[name="prenom_user"]').val(response.prenom_user);
