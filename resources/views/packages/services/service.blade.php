        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body" style="overflow: auto;">
                    <h5 class="card-title" style="text-align: center;font-size: 25px;">Listes des services de voiture
                    </h5>
                    <table class="table datatable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-success" data-service="{{ encodeId($service->id_service) }}"
                                        id="updateService">
                                        <i class="bi bi-pen"></i>
                                    </button>
                                    ...
                                    <button class="btn btn-danger" data-service="{{ encodeId($service->id_service) }}"
                                        id="deleteService">
...
<div class="modal fade" id="updateServiceModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('services-voitures.update') }}" method="post" id="services-voitures-update">
                @csrf
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <form action="{{ route('services-voitures.delete') }}" method="post" id="services-voitures-delete">
                    @csrf
...
@include('packages.services.serviceModal')
@endsection
...
<script>
    ...
    $(document).on('click', '#updateService', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '{{ route('services.info') }}',
            dataType: 'JSON',
            data: {id_service:id_service},
            success: (response)=>{
                $("#updateServiceModal").find('input[name="nom_service"]').val(response.nom_service);
                if(response.status_service){
                    $("#status_service").html('<option value="1" selected>Active</option><option value="0">Inactive</option>');
                }else{
                    $("#status_service").html('<option value="1">Active</option><option value="0" selected>Inactive</option>');
                }
                $(".update-service").html('<input type="hidden" name="id_service" value="'+response.id_service+'">');
    ...
    $(document).on('click', '#deleteService', function(){
        ...
        $.ajax({
            ...
            data: {id_service:id_service},
            success: (response)=>{
                $("#viewMessageDeleteService").html('Etes-vous sur de vouloir supprimer le service <strong>'+response.nom_service+'</strong>?');
                $("#formDeleteService").html('<input type="hidden" name="id_service" value="'+response.id_service+'"> <button type="submit" style="border-radius: 10px;" class="btn btn-danger btn-md"><i class="bx bx-trash"></i> Oui</button>');
