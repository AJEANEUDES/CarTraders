    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body row g-3">
                ...
                <div class="col-md-6 text-center">
                    <span>Status</span>
                    <div style="font-weight: 600;text-transform: uppercase;" id="viewStatusVoiture"></div>
                </div>
            </div>
...
<div class="modal fade" id="updateVoitureModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('voitures.update.gestion') }}" method="post" enctype="multipart/form-data" id="voitures-update-gestion">
                @csrf
                ...
                <div class="modal-body row g-3">
                    ...
                    <div class="col-md-6">
                        <label for="parking__id" class="form-label">Parc de la voiture</label>
                        <select style="height: 55px;border-radius: 10px;"
                            class="form-control form-control-lg" name="parking__id" id="parking__id">
                            {{-- @foreach ($parkings as $parking) --}}
                            <option selected value="{{ $parkings->id_parking }}">{{ Str::upper($parkings->nom_parking) }}</option>
                            {{-- @endforeach --}}
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select style="height: 55px;border-radius: 10px;text-transform: uppercase;"
                            ...
                            {{-- @foreach ($societes as $societe) --}}
                                <option selected value="{{ $societes->id_societe }}">{{ $societes->nom_societe }}</option>
                            {{-- @endforeach --}}
                        </select>
                    </div>
                    <div class="col-md-4">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <form action="{{ route('voitures.delete.gestion') }}" method="post" id="voitures-delete-gestion">
                    @csrf
...
@include('packages.voitures.gestionnaire.voitureModal')
@endsection
...
<script>
    ...
    $(document).ready(function(){
        $("#marque_id").on("change", function(){
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route('marques-voitures.get.spec.gestion') }}',
                dataType: 'JSON',
    ...
    $(document).ready(function(){
        $("#parking_id").on("change", function(){
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route('parkings-voitures.get.spec.gestion') }}',
                dataType: 'JSON',
                ...
                success: (response) => {
                    ...
                    let optionsSociete = '';
                    optionsSociete += '<option value="'+response.id_societe+'">'+ response.nom_societe +'</option>';
    ...
    $(document).on('click', '#getInfosVoiture', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '{{ route('voitures.info.gestion') }}',
            dataType: 'JSON',
            ...
            success: (response)=>{
                ...
                if(response.status_vente){
                    $("#viewStatusVoiture").html('<span class="badge bg-success">Vendue</span>');
                }else{
                    $("#viewStatusVoiture").html('<span class="badge bg-danger">Non Vendue</span>');
                }
    ...
    $(document).on('click', '#updateVoiture', function(){
        ...
        $.ajax({
            ...
            success: (response)=>{
                ...
                $("#updateVoitureModal").find('select[name="parking__id"]').val(response.parking_id);
                $("#updateVoitureModal").find('select[name="societe__id"]').val(response.societe_id);
