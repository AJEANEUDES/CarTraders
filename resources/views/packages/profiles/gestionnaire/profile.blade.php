        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                    ...
                    <form action="{{ route('gestionnaires.update.mdp') }}" method="post" id="gestionnaires-update-mdp">
                        @csrf
                        <input type="hidden" name="id_gestionnaire" class="id_gestionnaire" value="{{ encodeId(Auth::id()) }}">
                        <div class="row">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                    ...
                    <form action="{{ route('gestionnaires.profile.update') }}" method="post" enctype="multipart/form-data" id="gestionnaires-profile-update">
                        @csrf
                        <input type="hidden" name="id_gestionnaire" class="id_gestionnaire" value="{{ encodeId(Auth::id()) }}">
                        <div class="row">
                            ...
                            <div class="col-md-6 mt-2">
                                ...
                                <input type="text" style="height: 55px;border-radius: 10px;" name="prenom_user"
                                    id="prenom_user" class="form-control form-control-lg"
                                    value="" />
                            </div>
                            <div class="col-md-6 mt-2">
...
@push('scripts-inscription-check-countries')
    <script>
        $(document).ready(function(){
            let id_gestionnaire = $('.id_gestionnaire').val()
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route('gestionnaires.info.gestion') }}',
                dataType: 'JSON',
                data: {id_gestionnaire: id_gestionnaire},
                success: (response)=>{
                    $("#gestionnaires-profile-update").find('input[name="nom_user"]').val(response.nom_user);
                    $("#gestionnaires-profile-update").find('input[name="prenom_user"]').val(response.prenom_user);
                    $("#gestionnaires-profile-update").find('input[name="email_user"]').val(response.email_user);
                    $("#gestionnaires-profile-update").find('input[name="telephone_user"]').val(response.telephone_user);
                    $("#gestionnaires-profile-update").find('input[name="prefix_user"]').val(response.prefix_user);
                    $("#gestionnaires-profile-update").find('input[name="adresse_user"]').val(response.adresse_user);
                    $("#gestionnaires-profile-update").find('select[name="pays_user"]').val(response.pays_user);
                },
                error: (error)=>{
