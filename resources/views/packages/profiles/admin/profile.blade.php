        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                    ...
                    <form action="{{ route('admins.update.mdp') }}" method="post" id="admins-update-mdp">
                        @csrf
                        <input type="hidden" name="id_admin" class="id_admin" value="{{ encodeId(Auth::id()) }}">
                        <div class="row">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                    ...
                    <form action="{{ route('admins.profile.update') }}" method="post" enctype="multipart/form-data" id="admins-profile-update">
                        @csrf
                        <input type="hidden" name="id_admin" class="id_admin" value="{{ encodeId(Auth::id()) }}">
                        <div class="row">
    ...
    <script>
        $(document).ready(function(){
            ...
            $.ajax({
                ...
                success: (response)=>{
                    $("#admins-profile-update").find('input[name="nom_user"]').val(response.nom_user);
                    $("#admins-profile-update").find('input[name="prenom_user"]').val(response.prenom_user);
                    $("#admins-profile-update").find('input[name="email_user"]').val(response.email_user);
                    $("#admins-profile-update").find('input[name="telephone_user"]').val(response.telephone_user);
                    $("#admins-profile-update").find('input[name="prefix_user"]').val(response.prefix_user);
                    $("#admins-profile-update").find('input[name="adresse_user"]').val(response.adresse_user);
                    $("#admins-profile-update").find('select[name="pays_user"]').val(response.pays_user);
                },
                error: (error)=>{
