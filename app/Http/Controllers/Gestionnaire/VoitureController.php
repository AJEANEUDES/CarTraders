...
class VoitureController extends Controller
{
    public function getVoiture()
    {
        ...
        //     $id = $item->id_societe;
        //     $id_parking = $item->parking_id;
        // }
        $parkings = Parking::where('id_parking', $societes->parking_id)->where('status_parking', true)->first();

        $marques = Marque::where('status_marque', true)->get();
        $modeles = Modele::where('status_modele', true)->get();


        $voitures = Voiture::where('societe_id', $societes->id_societe)->where('status_vente', false)->orderByDesc('created_at')->get();
        $voitures_vendues = Voiture::where('societe_id', $societes->id_societe)
            ->where('status_vente', true)
            ->orderByDesc('created_at')->get();

        return view('packages.voitures.gestionnaire.voiture', compact([
